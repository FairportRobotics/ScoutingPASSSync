...
rootPath = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__), "data", f"{firstEventYear}", f"_{firstEventYear}"))
...
def fetch_events_for_season():
    ...
    # Log to file.
    save_file_to_subfolder(rootPath, f"{firstEventYear}.json", json.dumps(frcEvents, indent=3))
    ...
    for e_index, event in enumerate(frcEvents["Events"]):
        ...
        save_file_to_subfolder(rootPath, f"{firstEventYear}.{eventCode}.event.json", json.dumps(event, indent=3))
        ...
        save_file_to_subfolder(rootPath, f"{firstEventYear}.{eventCode}.schedule.json", json.dumps(frcMatches, indent=3))
        ...
        save_file_to_subfolder(rootPath, f"{firstEventYear}.{eventCode}.results.json", json.dumps(frcResults, indent=3))
        ...
        save_file_to_subfolder(rootPath, f"{firstEventYear}.{eventCode}.teams.json", json.dumps(frcTeams, indent=3))        
